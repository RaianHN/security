...
public class Mail 
{
	...
	public static void sendMailWithUNID(String strSubject,String strMessage,Vector strTo,String strSender,Document currentDoc ,boolean intThread )
	{
		...
		try{
			...
			Document Memo = Utility.getCurrentDatabase().createDocument();
			...
			if (!intThread)
			{
				...
				String url[]=Utility.getXspContext().getUrl().toString().split("\\?");
	...
	public static Database getCurrentUserMailDB()
	{
		...
		try 
		{
			...
			Document user1=  uview.getDocumentByKey(Utility.getCurrentSession().getEffectiveUserName());
