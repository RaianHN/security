...
/**
 *[This class contains utility methods which helps in getting Session specific
 * objects, ex current user name, current database]
 * 
 * @author VShashikumar
 *@created Oct 5, 2012
 */
public class SessionContext
{
	 ...
	 * 
	 * @param type
	 *            1 for abbreviated, 2 for common, 3 for canonical
	...
	public static String getFormattedName(int type)
	{
		...
		try
		{
			...
			Name name = ExtLibUtil.getCurrentSession().createName(
					db.getParent().getEffectiveUserName());
	...
	public static Map getViewScope()
	{
		...
		Map viewScope = (Map) context.getApplication().getVariableResolver()
				.resolveVariable(context, "viewScope");
		return viewScope;
	}
	...
	public static Map getSessionScope()
	{
		...
		FacesContext context = FacesContext.getCurrentInstance();
		Map sessionScope = (Map) context.getApplication().getVariableResolver()
				.resolveVariable(context, "sessionScope");
		return sessionScope;
	}

	/**
	 *[Returns the selected document from the UI by getting the document id set
	 * in the viewscope]
	 * 
	...
	public static Document getDocumentToProcess()
	{
		...
		if (classname.equalsIgnoreCase("java.util.vector"))
		{
			...
			try
			{
				...
				ret = currentdb.getDocumentByUNID((String) ((Vector) ids)
						.get(0).toString());
	...
	public static Document getDocumentToProcess(String dbName)
	{
		...
		if (classname.equalsIgnoreCase("java.util.vector"))
		{
			...
			try
			{
				...
				ret = Utility.getDatabase(dbName).getDocumentByUNID(
						(String) ((Vector) ids).get(0).toString());
	...
	/**
	 *[Returns the user profile document of the current user belonging to
	 * current database]
	 * 
	 * @return
	 */
	...
	public static Document getUserProfile()
	{
		...
		try
		{
			...
			String currentUser = ExtLibUtil.getCurrentSession()
