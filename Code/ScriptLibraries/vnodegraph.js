function NodeGraph(paper){
console.log("nodegraph");
  var win =dojo.window.getBox();
var node = "view:_id1:container";
  var canvas = dojo.query("#view:_id1:container");
  ...
  var key = {};
  var SHIFT = 16;
  var topHeight =  dojo.byId("controls").style.height;
  
 //var paper = dojox.gfx.createSurface(canvas, 200, 200);
...
//  function resizePaper(){
 ...
 // win.resize(resizePaper);
  ...
  var menu = dojo.query("#menu");
  //menu.css({"position" : "absolute", "left" : 100, "top" : 0, "z-index" : 5000, "border" : "1px solid gray", "padding" : 0});
 // menu.hide();
  
 dojo.query("#container").append("<div id='hit' />");
  //hitConnect = dojo.query("#hit");
  ...
  */
  ...
  function showOverlay(){
    overlay.show();
    overlay.css({"width" : win.w, "height" : win.h}); //, "opacity": 0.1});
  }
  ...
  function Node(xp, yp, w, h, noDelete, forceId){
   ...
   dojo.query("#container").append("<div class='node'/>");
    var n = dojo.query (".node").last();
    n.style({"position" : "absolute",      
           "left" : xp, "top" : yp,
           "width" : w, "height" : h,   
           "border" : "1px solid gray",
           "background-color" : "white"});
    n.style("z-index", zindex++);
    ...
    var nodeWidth = n.attr("width");
    var nodeHeight =n.attr("height");
           
    n.append("<div class='bar'/>");
   // n.innerHTML="<div class='bar'/>"
    
    
    var bar = dojo.query (".node .bar").last();
    ...
    if (!noDelete){
      ...
      var ex = dojo.query (".node .ex").last();
      ex.style({"position":"absolute","padding-right" : 2, "padding-top" : 1, "padding-left" : 2,
    ...
    var txt =dojo.query(".node .txt").last();
    txt.style("position","absolute");
   
    txt.style({"width" : nodeWidth - 5,
             "height" : nodeHeight - bar.attr("height") - 5,
    ...
    var resizer = dojo.query(".node .resizer").last();
    
    resizer.style({"position" : "absolute" , "z-index" : 10,
    ...
    var left = dojo.query(".node .left").last();
    left.style("left","-11px");
    
    var top =  dojo.query(".node .top").last();
    top.style("top","-11px");
    
    var right =  dojo.query(".node .right").last();
    var bottom =  dojo.query(".node .bottom").last();
    ...
    function positionLeft(){
      left.style("top", n.attr("height") / 2 - 5);
    }
    function positionRight(){
      right.style({"left":n.attr("width") + 1,"top": n.attr("height") / (2 - 5)});
    }
    function positionTop(){ 
      top.style("left", n.attr("width") / 2 - 5);
    }
    function positionBottom(){
      bottom.style({"top":n.attr("height") + 1,"left": n.attr("width") /( 2 - 5)});
    }
    
    function setupConnection(div){
      div.style({"position" : "absolute", "width" : "10px", "padding":0,
  ...
  // bottom.mousedown(addLink);
   ...
   /* resizer.mousedown(function(e){
    ...
    bar.mousedown(function(e){
      ...
      e.preventDefault();
      startDrag(n, {left : 10, top: 40, right : win.w - n.width() - 10, bottom : win.h - n.height() - 10},
      updateConnections);
    });
 ...
 function clear(){
  ...
  function defaultNode(){
    ...
    var temp = new Node(win.w / 2 - defaultWidth / 2, 
                        win.h / 2 - defaultHeight / 2,
