...
public class CommonActions {
	...
	public static void createDiscussion(Document currentdoc) {
		try {
			...
			Vector v = new Vector();
			String strSender = ExtLibUtil.getCurrentSession()
	...
	public static void sendReferenceRequest() {
		...
		try {
			...
			String sender = ExtLibUtil.getCurrentSession()
					.getEffectiveUserName();
	...
	private static Vector getUNIDSAndMarkDelete() {
		...
		try {
			...
			for (String id : arr) {
				Document selecteddoc = ExtLibUtil.getCurrentDatabase()
						.getDocumentByID(id);
	...
	private static boolean checkModuleForArchive() {
		...
		try {
			...
			module = Utility.getModuleProfile(mbdb, ExtLibUtil
